...
for i in range(20):
    ...
    for j in range(0,40,2):
        if s[i][j] == "0":
            ...
            number = s[i][j]+s[i][j+1]
        number_list.append(number)
    s[i] = map(int,number_list)

product_list = []

#Finding all horizontal products
for i in range(20):
    for j in range(17):
        product = s[i][j] * s[i][j+1]*s[i][j+2]*s[i][j+3]
        product_list.append(product)
    
 
#Finding all vertical products

for  i in range(17):
    for j in range(20):
        product = s[i][j] * s[i+1][j] * s[i+2][j] * s[i+3][j]
        product_list.append(product)



#Finding all products along negative sloped diagonals
...
#Finding all products along positive sloped diagonals
...
#Finding the maximum of the products

maximum_product = max(product_list)

print "The greatest product of 4 adjacent numbers is:", maximum_product 
